...
const AiringToday = () => {
  ...
  const logJSONData = async()=> {
    let response = await fetch(`https://api.themoviedb.org/3/tv/airing_today?api_key=${api_key}&language=en-US&page=${page}`);
  ...
  return (
    <div id="actors">
      ...
      <div id="actors-imgs-container">
        {movies.map((movie) => (
          <div key={movie.name}>
            <img className="actors-img" key={movie.name} src={`https://www.themoviedb.org/t/p/w220_and_h330_face/${movie.poster_path}`} alt="actor"/>
            <h3>{movie.name}</h3>
          </div>
        ))}
      </div>
